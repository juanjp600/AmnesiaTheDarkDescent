...
void StringResize(string& str, size_t sz)
{
	str.resize(sz);
}
...
void RegisterScriptString_Native(asIScriptEngine *engine)
{
	...
	if( sizeof(size_t) == 4 )
	{
		...
		r = engine->RegisterObjectMethod("string", "void resize(uint)", asFUNCTION(StringResize), asCALL_CDECL_OBJFIRST); assert( r >= 0 );
	}
	else
	{
		r = engine->RegisterObjectMethod("string", "uint64 length() const", asMETHOD(string,size), asCALL_THISCALL); assert( r >= 0 );
		r = engine->RegisterObjectMethod("string", "void resize(uint64)", asFUNCTION(StringResize), asCALL_CDECL_OBJFIRST); assert( r >= 0 );
	}
